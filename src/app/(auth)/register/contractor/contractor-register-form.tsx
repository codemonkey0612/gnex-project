...
const STEPS = ["企業情報", "サービス", "アカウント", "確認"];

type ServiceUnit = "UNIT_A" | "UNIT_B" | "UNIT_C" | "UNIT_D";
...
export function ContractorRegisterForm() {
  ...
  function toggleUnit(unit: ServiceUnit) {
    const current = form.serviceUnits;
    const next = current.includes(unit)
      ? current.filter((u) => u !== unit)
      : [...current, unit];
    updateField("serviceUnits", next);
  }
  ...
  function validateStep1(): boolean {
    const errs: FieldErrors = {};
    if (form.serviceUnits.length === 0)
      errs.serviceUnits = "対応Unitを1つ以上選択してください";
    ...
    return (
      <div className="rounded-lg border bg-card p-8 shadow-sm text-center">
        ...
        <Alert variant="success" className="mt-4">
          <AlertDescription>
            確認メールを送信しました。メール確認後、運営者による審査を行います。承認されるとサービスをご利用いただけます。
          </AlertDescription>
        </Alert>
        <Link href="/login">
  ...
  return (
    <div className="rounded-lg border bg-card p-8 shadow-sm">
      <div className="mb-2 text-center">
        <h1 className="text-2xl font-bold text-foreground">受注者 新規登録</h1>
      ...
      {step === 0 && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="description">企業紹介（任意）</Label>
            <textarea
              id="description"
              value={form.description}
              onChange={(e) => updateField("description", e.target.value)}
              placeholder="貴社の事業内容・強みなどをご記入ください"
              rows={3}
              maxLength={2000}
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>対応Unit *（複数選択可）</Label>
            <div className="space-y-2">
              {(
                Object.entries(SERVICE_UNIT_LABELS) as [string, string][]
              ).map(([key, label]) => (
                <label
                  key={key}
                  className={`flex cursor-pointer items-center gap-3 rounded-md border p-3 text-sm transition-colors ${
                    form.serviceUnits.includes(key as ServiceUnit)
                ...
                >
                  <input
                    type="checkbox"
                    checked={form.serviceUnits.includes(key as ServiceUnit)}
                    onChange={() => toggleUnit(key as ServiceUnit)}
            ...
            {errors.serviceUnits && (
              <p className="text-xs text-destructive">{errors.serviceUnits}</p>
            )}
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            ※ 資格・認定証のアップロードは、アカウント承認後にマイページから行えます。
          </p>
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="rounded-md border p-4 space-y-3 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">対応Unit</span>
              <div className="mt-1 flex flex-wrap gap-1">
                {form.serviceUnits.map((u) => (
                  <span
                    key={u}
                    className="rounded bg-primary/10 px-2 py-0.5 text-xs text-primary"
                  >
                    {u.replace("UNIT_", "Unit ")}
                  </span>
                ))}
              </div>
            </div>
          ...
          <Alert className="border-amber-200 bg-amber-50 text-amber-800">
            <AlertDescription>
              受注者アカウントは、メール確認後に運営者による審査があります。審査完了までお時間をいただく場合があります。
            </AlertDescription>
          </Alert>
